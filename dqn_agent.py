...
Transition = collections.namedtuple(
    'Transition', ['reward', 'observation', 'legal_actions', 'action', 'belief', 'begin'])
...
class DQNAgent(object):
  ...
  def __init__(self,
    ...
    with tf.device(tf_device):
      ...
      self._q, self._belief = online_convnet(
  ...
  def begin_episode(self, current_player, legal_actions, observation):
    ...
    self.action, self.belief = self._select_action(observation, legal_actions)
    self._record_transition(current_player, 0, observation, legal_actions,
                            self.action, self.belief, begin=True)
    return self.action, self.belief
  ...
  def _record_transition(self, current_player, reward, observation,
                         legal_actions, action, belief, begin=False):
  ...
  def _select_action(self, observation, legal_actions):
    ...
    action, belief = self._sess.run([self._q_argmax, self._belief],
    ...
    if random.random() <= epsilon:
      ...
      return np.random.choice(legal_action_indices[0]), belief[0]
    else:
      return action, belief[0]
