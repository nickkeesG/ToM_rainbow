...
def create_obs_stacker(environment, history_size=4):
  ...
  return ObservationStacker(history_size,
                            environment.vectorized_observation_shape()[0] + belief_size,
                            environment.players)
...
def parse_observations(observations, beliefs, num_actions, obs_stacker):
  ...
  belief_vector = beliefs[current_player]
  observation_vector = np.concatenate((observation_vector, belief_vector))
...
def run_one_episode(agent, environment, obs_stacker):
  ...
  beliefs = [np.zeros(512) for i in range(environment.players)]
  current_player, legal_moves, observation_vector = (
      parse_observations(observations, beliefs, environment.num_moves(), obs_stacker))
  action, belief = agent.begin_episode(current_player, legal_moves, observation_vector)
  beliefs[current_player] = belief
  ...
  while not is_done:
    ...
    if current_player in has_played:
      action, belief = agent.step(reward_since_last_action[current_player],
                          current_player, legal_moves, observation_vector)
      beliefs[current_player] = belief
    else:
      # Each player begins the episode on their first turn (which may not be
      # the first move of the game).
      action, belief = agent.begin_episode(current_player, legal_moves,
                                   observation_vector)
